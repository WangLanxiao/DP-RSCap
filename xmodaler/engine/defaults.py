...
class DefaultTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        # inputs={}
        # inputs['IMG_INPUT'] =  torch.randn(1,3, 224, 224).cuda()
        # inputs['G_TOKENS_TYPE'] = torch.stack([torch.tensor([1] * 25)]).cuda()
        # inputs['CLSAA_ID'] =[2]
        # inputs['IDS'] =  [torch.tensor(319).cuda()]
        # inputs['ENTITY_NAME'] =  'There are airport, runways, runway, planes, plane, square in image.'
        # # inputs['G_TOKENS_IDS'] =  torch.tensor([0]).cuda()
        # # inputs['TIME_STEP'] =  torch.tensor([0]).cuda()
        # inputs['G_TOKEN_EMBED'] =  torch.randn(1,512).cuda()
        ...
        self.val_data_loader = None  #self.build_val_loader(cfg)   #wlx
        # self.val_data_loader = self.build_val_loader(cfg)   #wlx
    ...
    def _write_metrics(
        ...
        if comm.is_main_process():
            ...
            if 'CrossEntropy Loss' in metrics_dict:
                storage.put_scalar("{}caption_loss".format(prefix), metrics_dict['CrossEntropy Loss'])
            if 'Class Loss' in metrics_dict:
                storage.put_scalar("{}class_loss".format(prefix), metrics_dict['Class Loss'])
            if 'CLIP Loss' in metrics_dict:
                storage.put_scalar("{}clip_loss".format(prefix), metrics_dict['CLIP Loss'])
            if 'LabelSmoothing(U) loss' in metrics_dict:
                storage.put_scalar("{}class_loss".format(prefix), metrics_dict['LabelSmoothing(U) loss'])
    ...
    def test(cls, cfg, model, test_data_loader, evaluator, epoch):
        ...
        with torch.no_grad():
            for data in tqdm.tqdm(test_data_loader):
                ...
                if 'CLS_RESULT' in res:
                    OUT_CLS=OUT_CLS+res['CLS_RESULT'].cpu().numpy()[:,0].tolist()
                    GT_CLS=GT_CLS+data['CLASS_ID'].cpu().numpy().tolist()
                for id, output in zip(ids, outputs):
                    results.append({cfg.INFERENCE.ID_KEY: int(id), cfg.INFERENCE.VALUE: output})

        if 'online' not in str(epoch):
            eval_res = evaluator.eval(results, epoch)
        else:
            eval_res = evaluator.eval_online(results, epoch)   #change for wlx for COCO online test
        if len(OUT_CLS)>0:
            # print(precision_score(GT_CLS, OUT_CLS, average='micro'))
            print(classification_report(GT_CLS, OUT_CLS))
        model.train()
        return eval_res
