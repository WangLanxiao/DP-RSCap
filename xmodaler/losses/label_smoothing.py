...
class LabelSmoothing(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.criterion_cl = nn.CrossEntropyLoss(ignore_index=-1)
        # self.bce = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([19],device="cuda:0"),reduction="mean")
    ...
    def forward(self, outputs_dict):
        ...
        if kfg.TREE_RESULT in outputs_dict:
            ...
            loss = F.binary_cross_entropy_with_logits(logits, targets,pos_weight=torch.tensor([19]).cuda(),reduction="mean")
        ...
        if 'CLS_PRE' in outputs_dict:
            ...
            loss_cls = self.criterion_cl(logits_cls, targets_cls)
            ret.update({'Class Loss': loss_cls* 1.5})
