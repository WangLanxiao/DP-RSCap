    ...
    def step(self, iteration: int, epoch: int, **kwargs: Any) -> None:
        ...
        if (iteration + 1) % self.period == 0:
            self.checkpointer.save(
                # "{}_Epoch_{:05d}".format(self.file_prefix, epoch), **additional_state
...
class XmodalerCheckpointer(Checkpointer):
    ...
    def __init__(self, model, save_dir="", *, save_to_disk=None,use_ema=False, **checkpointables):
    ...
    def _load_file(self, filename):
        ...
        if "model" not in loaded:
            loaded = {"model": loaded}
        # if self.use_ema and ('ema' in loaded['trainer']):
        #     ema_data=OrderedDict()
        #     for k in loaded['trainer']['ema']:
        #         name = k.replace('module.', '')
        #         ema_data[name] = loaded['trainer']['ema'][k]
        #     loaded['model'] = ema_data
        #     print("=====================  reload EMA model ========================")
        return loaded
