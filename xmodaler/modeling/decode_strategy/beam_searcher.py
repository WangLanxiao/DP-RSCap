...
from xmodaler.functional import expand_tensor, pad_tensor, dict_to_cuda
...
class BeamSearcher(DecodeStrategy):
    ...
    def _forward(self, batched_inputs, model):
        if kfg.IMG_INPUT in batched_inputs:
            batch_size = batched_inputs[kfg.IMG_INPUT].size(0)
        else:
        ...
        if kfg.IMG_INPUT in batched_inputs:
            backbone_out = model.backbone(inputs)
            inputs.update(backbone_out)
            # for idx,sub in enumerate(inputs['IDS']):
            #     path = '/data1/wlx/project/2022xmodaler/open_source_dataset/mscoco_dataset/swin_large/'+sub+'.npy'
            #     vvv = backbone_out['PYR_FEATS'][idx].cpu().numpy()
            #     np.save(path, vvv)
        ...
        ve_out = model.visual_embed(inputs)    #batched_inputs   wlx
        inputs.update(ve_out)

        encoder_out_v = model.encoder(inputs, mode='v')
        inputs.update(encoder_out_v)

        clip_out = model.backbone(batched_inputs, mode='clip')
        inputs.update(clip_out)

        masks = model.get_extended_attention_mask_clip(batched_inputs)
        inputs.update(masks)
                ...
                for key in expand_keys:
                    if key in inputs:
                        if isinstance(inputs[key], list):
                            ...
                            tensor=[]
                            for sss_pyr in inputs[key]:
                                tensor.append(expand_tensor(sss_pyr, beam_size))
                            a=1
                        else:
