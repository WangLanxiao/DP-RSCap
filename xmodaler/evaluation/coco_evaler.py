...
class COCOEvaler(object):
    ...
    def eval(self, results, epoch):
        # print('save online results into ', os.path.join(self.output_dir, str(epoch) + '_test.json'))
        if self.output_dir is not None:
            json.dump(results, open(os.path.join(self.output_dir, str(epoch) + '_test.json'), "w"))
    ...
    def eval_online(self, results, epoch):
        # print('save online results into ',os.path.join(self.output_dir, str(epoch) + '_online.json' ))
        if self.output_dir is not None:
            json.dump(results, open(os.path.join(self.output_dir, str(epoch) + '_online.json'), "w"))
        return ''